...
#include "binding/saveeasyrpgwindow.h"
...
class SaveEasyRpgData : public Binding::BindingBase {
	...
	Q_PROPERTY(int codepage READ codepage WRITE set_codepage NOTIFY codepage_changed)
	Q_PROPERTY(ArrayAdapter* windows READ windows CONSTANT)
...
public:
	...
	int codepage();
	void set_codepage(const int& new_codepage);
	ArrayAdapter* windows();

signals:
	void version_changed();
	void codepage_changed();

protected:
	lcf::rpg::SaveEasyRpgData& m_data;
	ArrayAdapter* m_windows;
};
} // namespace Binding::Generated
