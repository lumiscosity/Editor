...
namespace Binding::Generated {
	...
	QList<int> Map::generator_x() {
		return QList<int>(m_data.generator_x.begin(), m_data.generator_x.end());
	}
	void Map::set_generator_x(const QList<int>& new_generator_x) {
		if (QList<int>(m_data.generator_x.begin(), m_data.generator_x.end()) == new_generator_x)
			return;
		m_data.generator_x = decltype(m_data.generator_x)(new_generator_x.begin(), new_generator_x.end());
		emit generator_x_changed();
	}

	QList<int> Map::generator_y() {
		return QList<int>(m_data.generator_y.begin(), m_data.generator_y.end());
	}
	void Map::set_generator_y(const QList<int>& new_generator_y) {
		if (QList<int>(m_data.generator_y.begin(), m_data.generator_y.end()) == new_generator_y)
			return;
		m_data.generator_y = decltype(m_data.generator_y)(new_generator_y.begin(), new_generator_y.end());
		emit generator_y_changed();
	}

	QList<int> Map::generator_tile_ids() {
		return QList<int>(m_data.generator_tile_ids.begin(), m_data.generator_tile_ids.end());
	}
	void Map::set_generator_tile_ids(const QList<int>& new_generator_tile_ids) {
		if (QList<int>(m_data.generator_tile_ids.begin(), m_data.generator_tile_ids.end()) == new_generator_tile_ids)
			return;
	...
	QList<int> Map::lower_layer() {
		return QList<int>(m_data.lower_layer.begin(), m_data.lower_layer.end());
	}
	void Map::set_lower_layer(const QList<int>& new_lower_layer) {
		if (QList<int>(m_data.lower_layer.begin(), m_data.lower_layer.end()) == new_lower_layer)
			return;
		m_data.lower_layer = decltype(m_data.lower_layer)(new_lower_layer.begin(), new_lower_layer.end());
		emit lower_layer_changed();
	}

	QList<int> Map::upper_layer() {
		return QList<int>(m_data.upper_layer.begin(), m_data.upper_layer.end());
	}
	void Map::set_upper_layer(const QList<int>& new_upper_layer) {
		if (QList<int>(m_data.upper_layer.begin(), m_data.upper_layer.end()) == new_upper_layer)
			return;
