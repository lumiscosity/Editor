...
class TreeMap : public Binding::BindingBase {
	...
	Q_PROPERTY(QList<int> tree_order READ tree_order WRITE set_tree_order NOTIFY tree_order_changed)
...
public:
	...
	ArrayAdapter* maps();
	QList<int> tree_order();
	void set_tree_order(const QList<int>& new_tree_order);
	int active_node();
