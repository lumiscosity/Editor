...
namespace Binding::Generated {
	...
	QList<int> SaveActor::skills() {
		return QList<int>(m_data.skills.begin(), m_data.skills.end());
	}
	void SaveActor::set_skills(const QList<int>& new_skills) {
		if (QList<int>(m_data.skills.begin(), m_data.skills.end()) == new_skills)
			return;
		m_data.skills = decltype(m_data.skills)(new_skills.begin(), new_skills.end());
		emit skills_changed();
	}

	QList<int> SaveActor::equipped() {
		return QList<int>(m_data.equipped.begin(), m_data.equipped.end());
	}
	void SaveActor::set_equipped(const QList<int>& new_equipped) {
		if (QList<int>(m_data.equipped.begin(), m_data.equipped.end()) == new_equipped)
			return;
	...
	QList<int> SaveActor::battle_commands() {
		return QList<int>(m_data.battle_commands.begin(), m_data.battle_commands.end());
	}
	void SaveActor::set_battle_commands(const QList<int>& new_battle_commands) {
	...
	QList<int> SaveActor::status() {
		return QList<int>(m_data.status.begin(), m_data.status.end());
	}
	void SaveActor::set_status(const QList<int>& new_status) {
		if (QList<int>(m_data.status.begin(), m_data.status.end()) == new_status)
			return;
