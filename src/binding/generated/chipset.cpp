...
namespace Binding::Generated {
	...
	QList<int> Chipset::terrain_data() {
		return QList<int>(m_data.terrain_data.begin(), m_data.terrain_data.end());
	}
	void Chipset::set_terrain_data(const QList<int>& new_terrain_data) {
		if (QList<int>(m_data.terrain_data.begin(), m_data.terrain_data.end()) == new_terrain_data)
			return;
		m_data.terrain_data = decltype(m_data.terrain_data)(new_terrain_data.begin(), new_terrain_data.end());
		emit terrain_data_changed();
	}

	QList<int> Chipset::passable_data_lower() {
		return QList<int>(m_data.passable_data_lower.begin(), m_data.passable_data_lower.end());
	}
	void Chipset::set_passable_data_lower(const QList<int>& new_passable_data_lower) {
		if (QList<int>(m_data.passable_data_lower.begin(), m_data.passable_data_lower.end()) == new_passable_data_lower)
			return;
	...
	QList<int> Chipset::passable_data_upper() {
		return QList<int>(m_data.passable_data_upper.begin(), m_data.passable_data_upper.end());
	}
	void Chipset::set_passable_data_upper(const QList<int>& new_passable_data_upper) {
		if (QList<int>(m_data.passable_data_upper.begin(), m_data.passable_data_upper.end()) == new_passable_data_upper)
			return;
