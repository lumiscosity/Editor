...
namespace Binding::Generated {
	...
	QList<bool> SaveSystem::switches() {
		return QList<bool>(m_data.switches.begin(), m_data.switches.end());
	}
	void SaveSystem::set_switches(const QList<bool>& new_switches) {
		if (QList<bool>(m_data.switches.begin(), m_data.switches.end()) == new_switches)
			return;
		m_data.switches = decltype(m_data.switches)(new_switches.begin(), new_switches.end());
		emit switches_changed();
	}

	QList<int> SaveSystem::variables() {
		return QList<int>(m_data.variables.begin(), m_data.variables.end());
	}
	void SaveSystem::set_variables(const QList<int>& new_variables) {
		if (QList<int>(m_data.variables.begin(), m_data.variables.end()) == new_variables)
			return;
    ...
    // TODO: refactor the generator to point to this new function
	QList<QString> SaveSystem::maniac_strings() {
        return ToQStringgQList(m_data.maniac_strings);
	}
	void SaveSystem::set_maniac_strings(const QList<QString>& new_maniac_strings) {
        if (ToQStringgQList(m_data.maniac_strings) == new_maniac_strings)
			return;

        // TODO: this sucks bad also
        m_data.maniac_strings = std::vector<lcf::DBString>();
        for (auto i = new_maniac_strings.begin(); i != new_maniac_strings.end(); ++i) {
            m_data.maniac_strings.push_back(ToDBString(*i));
        }
		emit maniac_strings_changed();
	}
	...
	QList<int> SaveSystem::maniac_options() {
		return QList<int>(m_data.maniac_options.begin(), m_data.maniac_options.end());
	}
	void SaveSystem::set_maniac_options(const QList<int>& new_maniac_options) {
		if (QList<int>(m_data.maniac_options.begin(), m_data.maniac_options.end()) == new_maniac_options)
			return;
	...
	QList<int> SaveSystem::maniac_joypad_bindings() {
		return QList<int>(m_data.maniac_joypad_bindings.begin(), m_data.maniac_joypad_bindings.end());
	}
	void SaveSystem::set_maniac_joypad_bindings(const QList<int>& new_maniac_joypad_bindings) {
		if (QList<int>(m_data.maniac_joypad_bindings.begin(), m_data.maniac_joypad_bindings.end()) == new_maniac_joypad_bindings)
			return;
