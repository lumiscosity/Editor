...
namespace Binding::Generated {
	...
	QList<int> SaveInventory::party() {
		return QList<int>(m_data.party.begin(), m_data.party.end());
	}
	void SaveInventory::set_party(const QList<int>& new_party) {
		if (QList<int>(m_data.party.begin(), m_data.party.end()) == new_party)
			return;
		m_data.party = decltype(m_data.party)(new_party.begin(), new_party.end());
		emit party_changed();
	}

	QList<int> SaveInventory::item_ids() {
		return QList<int>(m_data.item_ids.begin(), m_data.item_ids.end());
	}
	void SaveInventory::set_item_ids(const QList<int>& new_item_ids) {
		if (QList<int>(m_data.item_ids.begin(), m_data.item_ids.end()) == new_item_ids)
			return;
		m_data.item_ids = decltype(m_data.item_ids)(new_item_ids.begin(), new_item_ids.end());
		emit item_ids_changed();
	}

	QList<int> SaveInventory::item_counts() {
		return QList<int>(m_data.item_counts.begin(), m_data.item_counts.end());
	}
	void SaveInventory::set_item_counts(const QList<int>& new_item_counts) {
		if (QList<int>(m_data.item_counts.begin(), m_data.item_counts.end()) == new_item_counts)
			return;
		m_data.item_counts = decltype(m_data.item_counts)(new_item_counts.begin(), new_item_counts.end());
		emit item_counts_changed();
	}

	QList<int> SaveInventory::item_usage() {
		return QList<int>(m_data.item_usage.begin(), m_data.item_usage.end());
	}
	void SaveInventory::set_item_usage(const QList<int>& new_item_usage) {
		if (QList<int>(m_data.item_usage.begin(), m_data.item_usage.end()) == new_item_usage)
			return;
