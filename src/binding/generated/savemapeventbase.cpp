...
namespace Binding::Generated {
	...
	int SaveMapEventBase::facing() {
		return m_data.facing;
	}
	void SaveMapEventBase::set_facing(const int& new_facing) {
		if (m_data.facing == new_facing)
			return;
		m_data.facing = new_facing;
		emit facing_changed();
	}
	...
	bool SaveMapEventBase::move_route_finished() {
		return m_data.move_route_finished;
	}
	void SaveMapEventBase::set_move_route_finished(const bool& new_move_route_finished) {
		if (m_data.move_route_finished == new_move_route_finished)
			return;
		m_data.move_route_finished = new_move_route_finished;
		emit move_route_finished_changed();
	}

	bool SaveMapEventBase::sprite_hidden() {
		return m_data.sprite_hidden;
	}
	void SaveMapEventBase::set_sprite_hidden(const bool& new_sprite_hidden) {
		if (m_data.sprite_hidden == new_sprite_hidden)
			return;
		m_data.sprite_hidden = new_sprite_hidden;
		emit sprite_hidden_changed();
	}

	bool SaveMapEventBase::move_route_through() {
		return m_data.move_route_through;
	}
	void SaveMapEventBase::set_move_route_through(const bool& new_move_route_through) {
		if (m_data.move_route_through == new_move_route_through)
			return;
		m_data.move_route_through = new_move_route_through;
		emit move_route_through_changed();
	}
